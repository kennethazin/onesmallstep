...
int MainFrame::extractDigit(const char* filename) {
    if (!filename) return -1;
    if (strstr(filename, "7SegOff")) return -1;

    int num;
    if (sscanf(filename, "7Seg-%d.jpg", &num) != 1) return -1;
  ...
  if (Channel == 010)
      ...
      if (Left != NULL)
	{
	  ...
	  if (SevenSegmentFilenames[i] == NULL)
	    goto Error;
	  if (SevenSegmentFilenames[i]) {
        int digit = MainWindow->extractDigit(SevenSegmentFilenames[i]);
        if (digit != -1) {
          if (Left == MainWindow->R3D2Digit) printf("R3D2: %d\n", digit);
          else if (Left == MainWindow->R3D4Digit) printf("R3D4: %d\n", digit);
        }
      }
      ...
      if (Right != NULL)
	{
	  ...
	  if (SevenSegmentFilenames[i]) {
        int digit = MainWindow->extractDigit(SevenSegmentFilenames[i]);
        if (digit != -1) {
          if (Right == MainWindow->R3D1Digit) printf("R3D1: %d\n", digit);
          else if (Right == MainWindow->R3D3Digit) printf("R3D3: %d\n", digit);
          else if (Right == MainWindow->R3D5Digit) printf("R3D5: %d\n", digit);
        }
      }
